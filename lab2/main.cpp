...
{
	size_t pos = val.get_name().find(name);
...
unordered_set<int> select_byfilter(unordered_map<int, pipe>& map)
{
	...
	case 1:
	{
		cout << "Enter part of name: "; string name = input_string();
		res = find_byfilter(map, find_by_name, name);
		break;
	}
	case 2:
	{
		cout << "Enter status \"in repair\": "; bool status = get_correct_number<int>(0, 1);
...
unordered_set<int> select_byfilter(unordered_map<int, CS>& map)
{
	...
	case 2:
	{
		cout << "Enter percent %: "; double percent = get_correct_number<double>(0.0, 100.0);
